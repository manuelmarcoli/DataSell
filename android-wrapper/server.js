...
app.use((req, res, next) => {
  ...
  if (req.path === '/api/health' || req.path === '/api/ping' || req.path === '/api/hubnet-webhook' || req.path === '/api/paystack-webhook' || req.path === '/config.js') {
    return next();
  }
...
app.get('/api/verify-payment/:reference', requireAuth, async (req, res) => {
  try {
    ...
    console.log('💳 Verifying payment with reference:', reference, 'for user:', userId);
    ...
    console.log('✅ Paystack response received:', result.status, result.data?.status);
    ...
    if (result.data.status === 'success') {
      ...
      console.log('💰 Payment successful! Original amount:', amount, 'User ID:', userId);
      ...
      console.log('💵 Updating wallet - Current:', currentBalance, 'Adding:', amount, 'New balance:', newBalance);
      
      // Credit the ORIGINAL amount - THIS IS THE KEY AUTOMATIC FUNDING
      ...
      try {
        ...
        const message = `hello ${username} your DataSell wallet has been credited with ₵${amount}. New balance: ₵${newBalance.toFixed(2)}. Thank you for choosing DataSell`;
        console.log('📱 Sending SMS to user:', phoneFallback);
        sendSmsToUser(userId, phoneFallback, message);
      } catch (smsErr) {
        console.error('⚠️ Wallet funding SMS error (but payment was credited):', smsErr.message);
        // Don't fail the payment if SMS fails
      }

      console.log('✅ PAYMENT VERIFICATION COMPLETE - Wallet credited with ₵' + amount);
      res.json({ 
        success: true, 
        amount: amount,
      ...
      console.warn('⚠️ Payment status is not success:', result.data.status);
      res.json({ 
        success: false, 
        error: 'Payment ' + (result.data.status || 'unknown') + '. If payment was successful, please contact support.' 
      });
    }
  } catch (error) {
...
app.post('/api/admin/users/:uid/delete', requireAdmin, async (req, res) => {
  try {
    const { uid } = req.params;
    ...
    // Fetch user data before deletion to log it
    const userRef = admin.database().ref('users/' + uid);
    const userSnapshot = await userRef.once('value');
    ...
    try {
      ...
      console.warn('Auth deletion error (user may not exist):', authError.message);
    }

    // Delete user from Realtime Database
    await userRef.remove();

    // Delete user's related data (transactions, orders, etc.)
    // Note: You can expand this to delete user-specific collections as needed
    const userTransactionsRef = admin.database().ref('transactions');
    const transactionsSnapshot = await userTransactionsRef.orderByChild('userId').equalTo(uid).once('value');
    
    if (transactionsSnapshot.exists()) {
      const updates = {};
      transactionsSnapshot.forEach(snap => {
        updates['/transactions/' + snap.key] = null;
      });
      await admin.database().ref().update(updates);
    }

    // Log admin action - IMPORTANT for audit trail
    const logRef = admin.database().ref('adminLogs').push();
    await logRef.set({
      adminId: req.session.user.uid,
      action: 'delete_user',
      ...
      details: `Permanently deleted user account for ${userEmail}. Reason: ${reason || 'Not specified'}`,
      timestamp: new Date().toISOString(),
      ip: req.ip,
      severity: 'critical'
    });

    console.log('✅ User account deleted successfully:', { uid, email: userEmail, deletedBy: req.session.user.uid });
...
// Paystack Webhook Handler - Automatically credits wallet when payment is confirmed
app.post('/api/paystack-webhook', async (req, res) => {
  try {
    ...
    if (!userId || !reference) {
      console.warn('⚠️ Webhook: Missing userId or reference');
      return res.status(400).json({ error: 'Missing userId or reference' });
    }

    console.log('✅ Processing Paystack payment - Reference:', reference, 'User:', userId, 'Amount: ₵' + originalAmount);
    ...
    if (paymentsSnapshot.exists()) {
      console.log('ℹ️ Webhook: Payment already processed for reference:', reference);
    ...
    const userData = userSnapshot.val();
    const currentBalance = userData.walletBalance || 0;
    const newBalance = currentBalance + parseFloat(originalAmount);
    ...
    await paymentRef.set({
      ...
      fee: (amount / 100) - parseFloat(originalAmount),
      reference,
      status: 'success',
    ...
    try {
      ...
      if (phone) {
        const message = `hello ${username} your DataSell wallet has been credited with ₵${originalAmount}. New balance: ₵${newBalance.toFixed(2)}. Thank you for choosing DataSell`;
        sendSmsToUser(userId, phone, message);
      }
    } catch (smsErr) {
      console.error('⚠️ Webhook: SMS error (payment still credited):', smsErr.message);
    }

    console.log('✅ WEBHOOK: Wallet credited with ₵' + originalAmount + ' for user ' + userId);
    res.status(200).json({ received: true, success: true, newBalance });
